...
def _get_size(shape, data_format):
    ...
    if len(shape) == 4:
        shape = shape[1:]
    return shape[1:3] if data_format == 'NCHW' else shape[0:2]  # NHWC


def _detection_layer(inputs, num_classes, anchors, img_size, data_format):
    """transforms raw predictions"""
...
def _upsample(inputs, out_shape, data_format='NCHW'):
    """Upsampled activations are concatenated along channels axis with activations from Darknet-53 layers."""
    # we need to pad with one pixel, so we set kernel_size = 3
    inputs = _fixed_padding(inputs, 3, mode='SYMMETRIC')  # the equivalent of edge mode

    # tf.image.resize_bilinear accepts input in format NHWC
    if data_format == 'NCHW':
        inputs = tf.transpose(inputs, [0, 2, 3, 1])  # change to default NHWC
